...
$username = $email = $password = $confirm_password = $admno = $course = $phone = "";
$username_err = $password_err = $email_err = $confirm_password_err  = $admno_err =  $course_err =  $phone_err = $dob_err = "";
...
if($_SERVER["REQUEST_METHOD"] == "POST")
{
    ...
    if(empty($username_err) && empty($password_err) && empty($confirm_password_err) && empty($email_err) && empty($admno_err) && empty($course_err) && empty($phone_err))
    {
        
        // Prepare an insert statement
        $sql = "INSERT INTO student (sname, email, admno, course, phone, password) VALUES (?, ?, ?, ?, ?, ?)";
         
        if($stmt = mysqli_prepare($link, $sql))
        {
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "ssssss", $param_username, $param_email, $param_admno, $param_course, $param_phone, $param_password);
            ...
            if(mysqli_stmt_execute($stmt))
            {
                ...
                window.location.href='stlogin.php';
                </script>";
...
<body style="background-image: url('index.jpeg'); background-repeat: no-repeat; background-attachment: fixed; background-size: 100% 100%;">
    <center>
